...
static struct option long_options[] = {
    ...
    {"invert-y", no_argument, 0, 'e'}, {"fps", required_argument, 0, 'f'},
    {"input-file", required_argument, 0, 'i'},
    {"camera", required_argument, 0, 'k'},
    {"icp-threshold", required_argument, 0, 'l'},
    {"log-file", required_argument, 0, 'o'}, {"mu", required_argument, 0, 'm'},
    {"init-pose", required_argument, 0, 'p'}, {"no-gui", no_argument, 0, 'q'},
    ...
    {"colour-voxels", no_argument, 0, 'C'}, {"multi-res", no_argument, 0, 'M'},
    {"bayesian", no_argument, 0, 'h'},
    {"ground-truth", required_argument, 0, 'g'},
    {"gt-transform", required_argument, 0, 'G'}, {0, 0, 0, 0}};

inline void print_arguments() {
    std::cerr << "-b  (--block-read)                        : default is "
                 "False: Block on read "
              << std::endl;
    std::cerr << "-c  (--compute-size-ratio)                : default is "
              << default_compute_size_ratio << "   (same size)      "
              << std::endl;
    std::cerr << "-e  (--invert-y)                          : default is "
                 "False: Block on read "
              << std::endl;
    std::cerr << "-d  (--dump-volume) <filename>            : Output volume "
                 "file              "
              << std::endl;
    std::cerr << "-f  (--fps)                               : default is "
              << default_fps << std::endl;
    std::cerr
        << "-F  (--bilateral-filter                   : default is disabled"
        << std::endl;
    std::cerr
        << "-h  (--bayesian                           : default is disabled"
        << std::endl;
    std::cerr << "-i  (--input-file) <filename>             : Input camera "
                 "file               "
              << std::endl;
    std::cerr << "-k  (--camera)                            : default is "
                 "defined by input     "
              << std::endl;
    std::cerr << "-l  (--icp-threshold)                     : default is "
              << default_icp_threshold << std::endl;
    std::cerr << "-o  (--log-file) <filename>               : default is "
                 "stdout               "
              << std::endl;
    std::cerr << "-m  (--mu)                                : default is "
              << default_mu << "               " << std::endl;
    std::cerr << "-p  (--init-pose)                         : default is "
              << default_initial_pos_factor.x() << ","
              << default_initial_pos_factor.y() << ","
              << default_initial_pos_factor.z() << "     " << std::endl;
    std::cerr << "-q  (--no-gui)                            : default is to "
                 "display gui"
              << std::endl;
    std::cerr << "-r  (--integration-rate)                  : default is "
              << default_integration_rate << "     " << std::endl;
    std::cerr << "-s  (--volume-size)                       : default is "
              << default_volume_size.x() << "," << default_volume_size.y()
              << "," << default_volume_size.z() << "      " << std::endl;
    std::cerr << "-t  (--tracking-rate)                     : default is "
              << default_tracking_rate << "     " << std::endl;
    std::cerr << "-v  (--volume-resolution)                 : default is "
              << default_volume_resolution.x() << ","
              << default_volume_resolution.y() << ","
              << default_volume_resolution.z() << "    " << std::endl;
    std::cerr
        << "-y  (--pyramid-levels)                    : default is 10,5,4     "
        << std::endl;
    std::cerr << "-z  (--rendering-rate)                    : default is "
              << default_rendering_rate << std::endl;
    std::cerr << "-g  (--ground-truth) <filename>           : Ground truth file"
              << std::endl;
    std::cerr << "-G  (--gt-transform) tx,ty,tz,qx,qy,qz,qw : Ground truth "
                 "pose tranform (translation and/or rotation)"
              << std::endl;
}
...
Configuration parseArgs(unsigned int argc, char** argv) {
    ...
    while ((c = getopt_long(argc, argv, short_options.c_str(), long_options,
        ...
        case 'a': {
            ...
            // initial_pose_quant = Eigen::Quaternionf(vals.w, vals.x, vals.y,
            // vals.z);
        ...
        case 'c': //   -c  (--compute-size-ratio)
            ...
            if ((config.compute_size_ratio != 1) &&
                ...
                std::cerr << "ERROR: --compute-size-ratio (-c) must be 1, 2 ,4 "
                             "or 8  (was "
                          << optarg << ")\n";
                flagErr++;
            }
        ...
        case 'g': // -g (--ground-truth)
            ...
            std::cerr << "using the groundtruth file "
                      << config.groundtruth_file << std::endl;
            break;
        case 'G': // -G (--gt-transform)
            ...
            case 4:
                ...
                gt_transform_quat = Eigen::Quaternionf(std::stof(tokens[3]),
                    ...
                    std::stof(tokens[2]));
                config.gt_transform.block<3, 3>(0, 0) =
                    gt_transform_quat.toRotationMatrix();
                break;
            case 7:
                ...
                break;
            default:
                std::cerr << "Invalid number of parameters for argument "
                             "gt-transform. Valid parameters are:\n"
                          << "3 parameters (translation): tx,ty,tz\n"
                          << "4 parameters (rotation in quaternion form): "
                             "qx,qy,qz,qw\n"
                          << "7 parameters (translation and rotation): "
        ...
        case 'p': //   -p  (--init-pose)
            ...
            break;
        case 'q': config.no_gui = true; break;
        ...
        case 's': //   -s  (--map-size)
            ...
            std::cerr << "update map_size to " << config.volume_size.x() << "mx"
                      << config.volume_size.y() << "mx"
                      << config.volume_size.z() << "m" << std::endl;
            if ((config.volume_size.x() <= 0) ||
                (config.volume_size.y() <= 0) ||
        ...
        case 'y': {
            ...
            std::cerr << "update pyramid levels to "
                      << pyramid2str(config.pyramid) << std::endl;
            break;
        case 'F':
        ...
        case '?':
            ...
            print_arguments();
            exit(0);
        default: std::cerr << "GetOpt abort."; flagErr = true;
        }

    if (flagErr) {
        std::cerr << "Exited due to " << flagErr << " error"
                  << (flagErr == 1 ? "" : "s") << " in command line options\n";
        exit(1);
    }
    return config;
}
