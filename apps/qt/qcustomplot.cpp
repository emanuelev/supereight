...
QCPLayoutElement::QCPLayoutElement(QCustomPlot *parentPlot) :
		...
		mParentLayout(0), mMinimumSize(), mMaximumSize(QWIDGETSIZE_MAX,
				QWIDGETSIZE_MAX), mRect(0, 0, 0, 0), mOuterRect(0, 0, 0, 0), mMargins(
				0, 0, 0, 0), mMinimumMargins(0, 0, 0, 0), mAutoMargins(
				QCP::msAll) {
}
...
QCPGrid::QCPGrid(QCPAxis *parentAxis) :
		QCPLayerable(parentAxis->parentPlot(), "", parentAxis), mParentAxis(
				parentAxis) {
...
QCPAxis::QCPAxis(QCPAxisRect *parent, AxisType type) :
		...
		mTickLabels(true), mAutoTickLabels(true), mTickLabelType(ltNumber), mTickLabelFont(
				...
				Qt::blue), mDateTimeFormat("hh:mm:ss\ndd.MM.yy"), mDateTimeSpec(
...
void QCPAxis::setNumberFormat(const QString &formatCode) {
	...
	if (formatCode.at(1) == 'b'
			&& (mNumberFormatChar == 'e' || mNumberFormatChar == 'g')) {
...
QCPAxisPainterPrivate::QCPAxisPainterPrivate(QCustomPlot *parentPlot) :
		type(QCPAxis::atLeft), basePen(
				...
				0), tickLabelPadding(0), tickLabelRotation(0), substituteExponent(
				true), numberMultiplyCross(false), tickLengthIn(5), tickLengthOut(
				0), subTickLengthIn(2), subTickLengthOut(0), tickPen(
				QPen(Qt::black, 0, Qt::SolidLine, Qt::SquareCap)), subTickPen(
...
void QCPAxisPainterPrivate::draw(QCPPainter *painter) {
	...
	if (!tickLabels.isEmpty()) {
		...
		for (int i = 0; i < maxLabelIndex; ++i)
			placeTickLabel(painter, tickPositions.at(i), margin,
	...
	if (!label.isEmpty()) {
		...
		if (type == QCPAxis::atLeft) {
			...
			painter->translate((origin.x() - margin - labelBounds.height()),
					origin.y());
			painter->rotate(-90);
			painter->drawText(0, 0, alignmentRect.height(),
			...
			painter->drawText(origin.x(),
					...
					alignmentRect.width(), labelBounds.height(),
	...
	int selTickLabelOffset = qMax(tickLengthOut, subTickLengthOut)
			+ tickLabelPadding;
...
void QCPAxisPainterPrivate::placeTickLabel(QCPPainter *painter, double position,
	...
	switch (type) {
	case QCPAxis::atLeft:
		labelAnchor = QPointF(alignmentRect.left() - distanceToAxis - offset,
				position);
		break;
	case QCPAxis::atRight:
		labelAnchor = QPointF(alignmentRect.right() + distanceToAxis + offset,
				position);
		break;
	case QCPAxis::atTop:
		labelAnchor = QPointF(position,
				alignmentRect.top() - distanceToAxis - offset);
		break;
	case QCPAxis::atBottom:
		labelAnchor = QPointF(position,
				alignmentRect.bottom() + distanceToAxis + offset);
		break;
	}
	if (mParentPlot->plottingHints().testFlag(QCP::phCacheLabels)
		...
		if (!mLabelCache.contains(text))  // no cached label exists, create it
			...
			newCachedLabel->offset = drawOffset
					+ labelData.rotatedTotalBounds.topLeft();
			newCachedLabel->pixmap = QPixmap(
					labelData.rotatedTotalBounds.size());
...
QCPAxisPainterPrivate::TickLabelData QCPAxisPainterPrivate::getTickLabelData(
	...
	if (useBeautifulPowers) {
		...
		if (abbreviateDecimalPowers && result.basePart == "1")
			...
			result.basePart += (
					numberMultiplyCross ?
							QString(QChar(215)) : QString(QChar(183))) + "10";
...
QCPAbstractPlottable::QCPAbstractPlottable(QCPAxis *keyAxis, QCPAxis *valueAxis) :
		QCPLayerable(keyAxis->parentPlot(), "", keyAxis->axisRect()), mName(""), mAntialiasedFill(
...
QCPItemPosition::QCPItemPosition(QCustomPlot *parentPlot,
		...
		QCPItemAnchor(parentPlot, parentItem, name), mPositionType(ptAbsolute), mKey(
				0), mValue(0), mParentAnchor(0) {
}
...
void QCPItemPosition::setType(QCPItemPosition::PositionType type) {
	if (mPositionType != type) {
		...
		if ((mPositionType == ptPlotCoords || type == ptPlotCoords)
				&& (!mKeyAxis || !mValueAxis))
...
QCPLegend::QCPLegend() {
	...
	setBorderPen (QPen(Qt::black));setSelectedBorderPen
	(QPen(Qt::blue, 2));
...
QCPPlotTitle::QCPPlotTitle(QCustomPlot *parentPlot) :
		QCPLayoutElement(parentPlot), mFont(
				QFont("sans serif", 13 * 1.5, QFont::Bold)), mTextColor(
				Qt::black), mSelectedFont(
				QFont("sans serif", 13 * 1.6, QFont::Bold)), mSelectedTextColor(
...
key(key), value(value), keyErrorPlus(0), keyErrorMinus(0), valueErrorPlus(0), valueErrorMinus(
	0) {
}
...
setErrorPen (QPen(Qt::black));setBrush
(Qt::NoBrush);
...
painter->fillRect(
	QRectF(rect.left(), rect.top() + rect.height() / 2.0, rect.width(),
			rect.height() / 3.0), mBrush);
}
...
scaledStyle.setPixmap(
		scaledStyle.pixmap().scaled(rect.size().toSize(), Qt::KeepAspectRatio,
				Qt::SmoothTransformation));
...
int keyPixelSpan = qAbs(
	keyAxis->coordToPixel(lower.key()) - keyAxis->coordToPixel(upper.key()));
...
if (!addedLastAlready) // in case curve just entered R, make sure the last point outside R is also drawn correctly
	lineData->append(
			coordsToPixels((it - 1).value().key, (it - 1).value().value)); // add last point to vector
...
if (lastRegion == 5 || // coming from R
		((lastRegion == 2 && currentRegion == 4)
				|| (lastRegion == 4 && currentRegion == 2)) || // skip top left diagonal
		((lastRegion == 4 && currentRegion == 8)
				|| (lastRegion == 8 && currentRegion == 4)) || // skip top right diagonal
...
} else // no special case that forbids optimized point placement, so do it:
{
	...
	if (!addedLastAlready)
		lineData->append(
				outsideCoordsToPixels((it - 1).value().key,
						(it - 1).value().value, currentRegion, axisRect));
	// add current point, optimized:
	lineData->append(
			outsideCoordsToPixels(it.value().key, it.value().value,
...
lineData->append(
	coordsToPixels((mData->constEnd() - 1).value().key,
...
return;
if (bars
&& (bars->keyAxis() != mKeyAxis.data() || bars->valueAxis() != mValueAxis.data())) {
qDebug() << Q_FUNC_INFO
...
if (inSignDomain == sdBoth
	|| (inSignDomain == sdNegative && current + barWidthHalf < 0)
...
setOutliers (QVector<double>());setKey(0);
setMinimum(0);
setLowerQuartile(0);
setMedian(0);
setUpperQuartile(0);
...
QCPItemStraightLine::QCPItemStraightLine(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), point1(createPosition("point1")), point2(
				createPosition("point2")) {
...
QCPItemLine::QCPItemLine(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), start(createPosition("start")), end(
				createPosition("end")) {
	start->setCoords(0, 0);
	end->setCoords(1, 1);
...
QCPItemCurve::QCPItemCurve(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), start(createPosition("start")), startDir(
				createPosition("startDir")), endDir(createPosition("endDir")), end(
				createPosition("end")) {
	start->setCoords(0, 0);
...
QCPItemRect::QCPItemRect(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), topLeft(createPosition("topLeft")), bottomRight(
				createPosition("bottomRight")), top(createAnchor("top", aiTop)), topRight(
				createAnchor("topRight", aiTopRight)), right(
				createAnchor("right", aiRight)), bottom(
				createAnchor("bottom", aiBottom)), bottomLeft(
				createAnchor("bottomLeft", aiBottomLeft)), left(
				createAnchor("left", aiLeft)) {
...
QCPItemText::QCPItemText(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), position(createPosition("position")), topLeft(
				createAnchor("topLeft", aiTopLeft)), top(
				createAnchor("top", aiTop)), topRight(
				createAnchor("topRight", aiTopRight)), right(
				createAnchor("right", aiRight)), bottomRight(
				createAnchor("bottomRight", aiBottomRight)), bottom(
				createAnchor("bottom", aiBottom)), bottomLeft(
				createAnchor("bottomLeft", aiBottomLeft)), left(
				createAnchor("left", aiLeft)) {
...
QCPItemEllipse::QCPItemEllipse(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), topLeft(createPosition("topLeft")), bottomRight(
				createPosition("bottomRight")), topLeftRim(
				createAnchor("topLeftRim", aiTopLeftRim)), top(
				createAnchor("top", aiTop)), topRightRim(
				createAnchor("topRightRim", aiTopRightRim)), right(
				createAnchor("right", aiRight)), bottomRightRim(
				createAnchor("bottomRightRim", aiBottomRightRim)), bottom(
				createAnchor("bottom", aiBottom)), bottomLeftRim(
				createAnchor("bottomLeftRim", aiBottomLeftRim)), left(
				createAnchor("left", aiLeft)), center(
				createAnchor("center", aiCenter)) {
	topLeft->setCoords(0, 1);
	bottomRight->setCoords(1, 0);
...
QCPItemPixmap::QCPItemPixmap(QCustomPlot *parentPlot) :
	...
	setPen(Qt::NoPen);
	setSelectedPen (QPen(Qt::blue));setScaled
	(false, Qt::KeepAspectRatio);
}
...
QCPItemTracer::QCPItemTracer(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), position(createPosition("position")), mGraph(
				0) {
	position->setCoords(0, 0);
...
QCPItemBracket::QCPItemBracket(QCustomPlot *parentPlot) :
		QCPAbstractItem(parentPlot), left(createPosition("left")), right(
				createPosition("right")), center(
				createAnchor("center", aiCenter)) {
	left->setCoords(0, 0);
	right->setCoords(1, 1);
...
double QCPItemBracket::selectTest(const QPointF &pos, bool onlySelectable,
	...
	return qSqrt(
			distSqrToLine((centerVec - widthVec).toPointF(),
