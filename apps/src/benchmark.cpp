...
int main(int argc, char** argv) {
    ...
    if (is_file(config.input_file)) {
        reader = new RawDepthReader(
            config.input_file, config.fps, config.blocking_read);

    } else {
        reader = new SceneDepthReader(
            config.input_file, config.fps, config.blocking_read);
    }
    ...
    Eigen::Vector3f init_pose =
        config.initial_pos_factor.cwiseProduct(config.volume_size);
    ...
    *logstream << "frame\tacquisition\tpreprocessing\ttracking\tintegration\tra"
                  "ycasting\trendering\tcomputation\ttotal    \tX          \tY "
    ...
    while (reader->readNextDepthFrame(inputDepth)) {
        ...
        tracked = pipeline.tracking(
            camera, config.icp_threshold, config.tracking_rate, frame);
        ...
        if (tracked || (frame <= 3)) {
            integrated = pipeline.integration(
                camera, config.integration_rate, config.mu, frame);
        } else {
            integrated = false;
        }
