...
static vertex_index parseTriple(
    const char*& token, int vsize, int vnsize, int vtsize) {
    vertex_index vi(-1);
...
static bool exportFaceGroupToShape(shape_t& shape,
    ...
    if (faceGroup.empty()) { return false; }
    ...
    for (size_t i = 0; i < faceGroup.size(); i++) {
        ...
        for (size_t k = 2; k < npolys; k++) {
            ...
            unsigned int v0 = updateVertex(vertexCache, shape.mesh.positions,
                shape.mesh.normals, shape.mesh.texcoords, in_positions,
                in_normals, in_texcoords, i0);
            unsigned int v1 = updateVertex(vertexCache, shape.mesh.positions,
                shape.mesh.normals, shape.mesh.texcoords, in_positions,
                in_normals, in_texcoords, i1);
            unsigned int v2 = updateVertex(vertexCache, shape.mesh.positions,
                shape.mesh.normals, shape.mesh.texcoords, in_positions,
...
bool LoadObj(std::vector<shape_t>& shapes, // [output]
    ...
    std::string basePath;
    if (mtl_basepath) { basePath = mtl_basepath; }
