...
void qtLinkKinectQt(int argc,
                    char *argv[],
                    DenseSLAMSystem **_pipeline,
                    DepthReader **_depthReader,
                    Configuration *config,
                    void *depthRender,
                    void *trackRender,
                    void *volumeModel,
...
int main(int argc, char **argv) {
  ...
  inputDepth = (uint16_t *) malloc(sizeof(uint16_t) * inputSize.x * inputSize.y);
  inputRGB = (uchar3 *) malloc(sizeof(uchar3) * inputSize.x * inputSize.y);
  depthRender = (uchar4 *) malloc(sizeof(uchar4) * computationSize.x * computationSize.y);
  trackRender = (uchar4 *) malloc(sizeof(uchar4) * computationSize.x * computationSize.y);
  volumeRender = (uchar4 *) malloc(sizeof(uchar4) * computationSize.x * computationSize.y);

  init_pose = config.initial_pos_factor.cwiseProduct(config.volume_size);
  pipeline = new DenseSLAMSystem(Eigen::Vector2i(computationSize.x, computationSize.y),
  ...
  if (config.dump_volume_file != "") {
    ...
    Stats.sample("meshing", std::chrono::duration<double>(end - start).count(), PerfStats::TIME);
  }
...
//  std::vector<Eigen::Vector3i>* frontier_blocks;
  ...
  Eigen::Vector4f camera = (reader != NULL) ? reader->getK() : Eigen::Vector4f::Constant(0.0f);
  ...
  if (processFrame) {
    ...
    if (config->groundtruth_file == "") {
      ...
      tracked = pipeline->tracking(camera, config->icp_threshold, config->tracking_rate, frame);
    } else {
  ...
  if (renderImages) {
    ...
    pipeline->renderVolume((unsigned char *) volumeRender,
                           pipeline->getComputationResolution(),
  ...
  if (config->no_gui) {
    *logstream << reader->getFrameNumber() << "\t" << xt << "\t" << yt << "\t" << zt << "\t"
               << std::endl;
  }
