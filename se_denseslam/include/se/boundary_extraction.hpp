...
void insertFrontierBlocksToMap(map3i &frontier_blocks_map, const vec3i *frontier_blocks) {
  if(frontier_blocks->size()!=NULL) {
  std::cout << "[supereight/boundary] adding " << frontier_blocks->size() << " new frontier blocks "
                                                                           "" <<
  std::endl;
  } else{
...
//    se::siblings(siblings_res, octant_curr_voxel, max_depth);
    ...
    for (int col = 0 ; col < 6 ; col ++){
      ...
      if(volume._map_index->get(face_res.block<3,1>(0,col)).y != 0.0 && volume._map_index->get
      (face_res.block<3,1>(0,col)).x <0.0){
        is_frontier = true;
      }
    }
...
void updateFrontierMap(const Volume<T> &volume, map3i &frontier_blocks_map) {
  ...
  for (auto it = frontier_blocks_map.begin(); it != frontier_blocks_map.end(); ++it){
    ...
    if(volume._map_index->get(it->first).st != voxel_state::kFrontier){
       std::cout << "[supereight/boundary] not frontier => erase" << std::endl;
...
template  <typename T>
void updateFrontierMap(const Volume<T> &volume, map3i &frontier_blocks_map, vec3i
*frontier_blocks) {
