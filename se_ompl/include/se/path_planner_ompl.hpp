...
class PathPlannerOmpl {
 public:
   ...
   */
  PathPlannerOmpl(const Octree<FieldType> & octree,
...
emplate<typename FieldType>
bool PathPlannerOmpl<FieldType>::setupPlanner(const Eigen::Vector3d &start_m, const Eigen::Vector3d &goal_m) {
...
//  ow_->GetMapBoundsMeter(min_boundary, max_boundary);
  ...
  std::shared_ptr<StateValidityChecker <FieldType> > state_validity_checker
      (new StateValidityChecker<FieldType>(ss_.getSpaceInformation(), pcc_,
...
//      (std::shared_ptr<InformationGainObjective>(new InformationGainObjective(ss_.getSpaceInformation())));
    ob::OptimizationObjectivePtr objective
        (std::make_shared<ob::PathLengthOptimizationObjective>(ss_.getSpaceInformation()));
//    ss_.getProblemDefinition()->setOptimizationObjective(information_gain_obj);
...
template<typename FieldType>
bool PathPlannerOmpl<FieldType>::planPath(const Eigen::Vector3d &start_m, const Eigen::Vector3d &goal_m) {
